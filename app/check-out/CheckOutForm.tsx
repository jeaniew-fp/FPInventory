...
function CheckOutFormInner() {
  ...
  useEffect(() => {
    ...
    if (itemId) loadItemById(itemId);
  }, [searchParams]);

  async function loadItemById(itemId: string) {
    const { data } = await supabase.from('inventory_items').select('*').eq('id', itemId).single();
    if (data) { setSelectedItem(data); setQuantity(''); setItemSearch(''); setItemResults([]); }
    else toast.error('Item not found. Please search manually.');
  }
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      if (uuidRegex.test(itemSearch.trim())) {
        const { data } = await supabase.from('inventory_items').select('*').eq('id', itemSearch.trim()).single();
        setItemResults(data ? [data] : []);
      } else {
        const { data } = await supabase.from('inventory_items').select('*')
          .or(`description.ilike.%${itemSearch}%,category.ilike.%${itemSearch}%`)
          .gt('current_quantity', 0).order('description').limit(10);
        setItemResults(data ?? []);
      }
    }, 300);
  ...
  function resetForm() {
    setItemSearch(''); setItemResults([]); setSelectedItem(null);
    setClientFirstName(''); setClientLastName(''); setHmisNumber('');
    setCaseManagerId(''); setProgram(''); setQuantity('');
    setDateGiven(format(new Date(), 'yyyy-MM-dd'));
    setFamilySize(''); setShelterLocation(''); setGiftCardPurpose('');
    setSuccess(false); setSubmittedData(null);
  }
  ...
  async function handlePrintReceipt() {
    ...
    function labelValue(label: string, value: string) {
      ...
      doc.setFont('helvetica', 'normal'); doc.text(value, margin + 90, y); y += 16;
    }

    doc.setFontSize(11); doc.setFont('helvetica', 'bold'); doc.setTextColor(0, 99, 190);
    doc.text('CLIENT INFORMATION', margin, y); y += 14;
    ...
    doc.setFontSize(11); doc.setFont('helvetica', 'bold'); doc.setTextColor(0, 99, 190);
    doc.text('GIFT CARD DETAILS', margin, y); y += 14;
    doc.setTextColor(30, 30, 30); doc.setFont('helvetica', 'normal'); doc.setFontSize(10);
    const faceValue = submittedData.item.face_value ?? 0;
    const qty = submittedData.quantity;
    labelValue('Retailer / Store', submittedData.item.retailer ?? 'Gift Card');
    labelValue('Face Value per Card', `$${faceValue.toFixed(2)}`);
    labelValue('Number of Cards', `${qty}`);
    doc.setFillColor(240, 247, 255); doc.setDrawColor(0, 99, 190);
    doc.roundedRect(margin, y, contentW, 28, 4, 4, 'FD');
    doc.setFont('helvetica', 'bold'); doc.setFontSize(12); doc.setTextColor(0, 99, 190);
    doc.text(`Total Value Given: $${(faceValue * qty).toFixed(2)}`, pageW / 2, y + 18, { align: 'center' });
    y += 40; doc.setTextColor(30, 30, 30); doc.setFont('helvetica', 'normal'); doc.setFontSize(10);
    if (submittedData.giftCardPurpose) labelValue('Purpose', submittedData.giftCardPurpose);
    y += 6; doc.line(margin, y, pageW - margin, y); y += 16;

    doc.setFontSize(11); doc.setFont('helvetica', 'bold'); doc.setTextColor(0, 99, 190);
    doc.text('STAFF INFORMATION', margin, y); y += 14;
    doc.setTextColor(30, 30, 30); doc.setFont('helvetica', 'normal'); doc.setFontSize(10);
    ...
    doc.text('Client Signature', margin, y + 42); doc.text('Date', margin + 160, y + 42);
    const sig2X = margin + 260;
    doc.line(sig2X, y + 30, sig2X + 200, y + 30);
    doc.text('Staff Signature', sig2X, y + 42); doc.text('Date', sig2X + 160, y + 42);

    doc.setFillColor(245, 245, 245); doc.rect(0, 792 - 40, pageW, 40, 'F');
    doc.setFontSize(8); doc.setTextColor(120, 120, 120);
    doc.text('Family Promise of Greater Washington County  ·  This receipt confirms the distribution of gift card(s) to the above-named client.', pageW / 2, 792 - 24, { align: 'center' });
  ...
  if (success && submittedData) {
    ...
    return (
      <div className="flex items-center justify-center py-12 px-4">
        <div className="bg-white rounded-2xl p-8 text-center max-w-sm w-full" style={cardStyle}>
          <div className="w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-4" style={{ backgroundColor: isGC ? '#f3e6f5' : '#fff3e0' }}>
            <span className="text-3xl">{isGC ? '🎁' : '📤'}</span>
          </div>
          <h2 className="text-xl font-extrabold mb-2" style={{ color: '#3a1f4a' }}>Check-Out Complete</h2>
          {isGC ? (
            <>
              <p className="text-sm mb-1" style={{ color: '#5d5468' }}>
                {submittedData.quantity}x <strong>{submittedData.item.retailer}</strong> gift card{submittedData.quantity !== 1 ? 's' : ''}
              </p>
              <p className="font-semibold text-sm mb-1" style={{ color: '#8b4a72' }}>
          ...
          <p className="text-sm mb-6" style={{ color: '#8a7f92' }}>
            given to <strong style={{ color: '#3a1f4a' }}>{submittedData.clientFirstName} {submittedData.clientLastName}</strong>
          </p>
          <div className="space-y-3">
            {isGC && (
              <button onClick={handlePrintReceipt}
                className="w-full text-white py-3 rounded-xl font-semibold transition-opacity hover:opacity-90"
                style={{ background: 'linear-gradient(130deg,#1b6cb0,#3f9be0)' }}>
    ...
    <div className="max-w-lg mx-auto pb-10">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="bg-white rounded-2xl p-6" style={cardStyle}>
          ...
          {!selectedItem ? (
            <>
              ...
              <div>
                <label className="block text-sm font-semibold mb-1.5" style={{ color: '#4d4457' }}>Search by item name or category</label>
                ...
                <div className="mt-2 rounded-xl overflow-hidden" style={{ border: '1.5px solid #e3ddec' }}>
                  {itemResults.map(item => (
                    <button key={item.id} type="button"
                      onClick={() => { setSelectedItem(item); setQuantity(''); setItemSearch(''); setItemResults([]); }}
                      ...
                      <div className="flex justify-between items-start">
                        <div>
                          <p className="font-semibold text-sm" style={{ color: '#2a2333' }}>{item.item_type === 'gift_card' ? '🎁 ' : ''}{item.description}</p>
                        ...
                        <span className={`text-xs font-semibold px-2 py-1 rounded-full ml-2 ${
                          item.current_quantity > 5 ? 'bg-green-100 text-green-700'
                          : item.current_quantity > 0 ? 'bg-yellow-100 text-yellow-700'
                          : 'bg-red-100 text-red-600'
                        }`}>{item.current_quantity} avail.</span>
                      </div>
                    </button>
                  ))}
                </div>
              ...
              {itemSearch.length >= 2 && itemResults.length === 0 && (
                <p className="text-sm mt-2" style={{ color: '#9a8fa0' }}>No items found. Try a different search term.</p>
              )}
            </>
          ) : (
            <div className="rounded-xl p-4 flex items-start justify-between"
              style={isGiftCard ? { background: '#f3e6f5', border: '1px solid #d6cee0' } : { background: '#fff3e0', border: '1px solid #fdd8a0' }}>
              <div>
                <p className="font-semibold" style={{ color: '#3a1f4a' }}>{isGiftCard ? '🎁 ' : ''}{selectedItem.description}</p>
                {isGiftCard && selectedItem.face_value && <p className="text-sm" style={{ color: '#8b4a72' }}>${selectedItem.face_value.toFixed(2)} per card</p>}
                <p className="text-xs mt-0.5" style={{ color: isGiftCard ? '#8b4a72' : '#b5720a' }}>{selectedItem.storage_location}</p>
                <p className="text-sm font-semibold mt-1" style={{ color: isGiftCard ? '#3a1f4a' : '#7a4a10' }}>{selectedItem.current_quantity} available</p>
              </div>
              <button type="button" onClick={() => setSelectedItem(null)} className="ml-3 text-sm" style={{ color: '#8b4a72' }}>✕</button>
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-2xl p-6 space-y-4" style={cardStyle}>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="text" required value={clientFirstName} onChange={e => setClientFirstName(e.target.value)}
                className="w-full px-4 py-3 rounded-xl text-base focus:outline-none focus:ring-2 focus:ring-purple-300" style={inputStyle} />
            </div>
            <div>
              <label className="block text-sm font-semibold mb-1.5" style={{ color: '#4d4457' }}>Last Name <span className="text-red-500">*</span></label>
              <input type="text" required value={clientLastName} onChange={e => setClientLastName(e.target.value)}
                className="w-full px-4 py-3 rounded-xl text-base focus:outline-none focus:ring-2 focus:ring-purple-300" style={inputStyle} />
            </div>
          </div>
          <div>
            <label className="block text-sm font-semibold mb-1.5" style={{ color: '#4d4457' }}>HMIS Number (optional)</label>
            <input type="text" value={hmisNumber} onChange={e => setHmisNumber(e.target.value)} placeholder="e.g., 12345"
              className="w-full px-4 py-3 rounded-xl text-base focus:outline-none focus:ring-2 focus:ring-purple-300" style={inputStyle} />
          </div>
          <div>
            ...
            <select required value={caseManagerId} onChange={e => setCaseManagerId(e.target.value)}
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="number" required min="1" max={selectedItem?.current_quantity ?? 999} value={quantity}
                    onChange={e => setQuantity(e.target.value === '' ? '' : Math.max(1, parseInt(e.target.value) || 1))}
                    className="w-full px-4 py-3 rounded-xl text-base focus:outline-none focus:ring-2 focus:ring-purple-300" style={inputStyle} />
                  {selectedItem && quantity > selectedItem.current_quantity && <p className="text-xs text-red-500 mt-1">Exceeds available quantity</p>}
                </div>
                <div>
                  <label className="block text-sm font-semibold mb-1.5" style={{ color: '#4d4457' }}>Family Size <span className="text-red-500">*</span></label>
                  <input type="number" required min="1" value={familySize}
              ...
              <div>
                <label className="block text-sm font-semibold mb-1.5" style={{ color: '#4d4457' }}>Shelter Location <span className="text-red-500">*</span></label>
                <select required value={shelterLocation} onChange={e => setShelterLocation(e.target.value)}
                  className="w-full px-4 py-3 rounded-xl text-base focus:outline-none focus:ring-2 focus:ring-purple-300" style={selectStyle}>
              ...
              <div>
                <label className="block text-sm font-semibold mb-1.5" style={{ color: '#4d4457' }}>Purpose <span className="text-red-500">*</span></label>
                <select required value={giftCardPurpose} onChange={e => setGiftCardPurpose(e.target.value)}
                  className="w-full px-4 py-3 rounded-xl text-base focus:outline-none focus:ring-2 focus:ring-purple-300" style={selectStyle}>
          ...
          {!isGiftCard && (
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="block text-sm font-semibold mb-1.5" style={{ color: '#4d4457' }}>Date Given</label>
        ...
        <button
          type="submit"
          disabled={
            loading || !selectedItem || !clientFirstName.trim() || !clientLastName.trim() ||
            !caseManagerId || !program || quantity < 1 ||
            (selectedItem ? quantity > selectedItem.current_quantity : false) ||
            (isGiftCard && (!shelterLocation || !giftCardPurpose))
          }
          className="w-full text-white py-3.5 rounded-xl font-extrabold text-base disabled:opacity-40 disabled:cursor-not-allowed transition-opacity hover:opacity-90"
          style={isGiftCard
            ? { background: 'linear-gradient(120deg,#f5a623,#d1618a 55%,#7c3a9e)', boxShadow: '0 14px 30px -14px rgba(124,58,158,.7)' }
            : { background: 'linear-gradient(130deg,#e87d4a,#f5a623)', boxShadow: '0 14px 30px -14px rgba(232,125,74,.6)' }
          }
        >
