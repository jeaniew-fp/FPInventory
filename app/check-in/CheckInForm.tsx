...
function CheckInFormInner() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      const { data } = await supabase.from('donors').select('*').ilike('name', `%${donorSearch}%`).limit(8);
  ...
  useEffect(() => {
    if (category && condition) setFmvPerUnit(estimateFMV(category, condition));
  }, [category, condition]);
  ...
  async function saveDonorIfNew(): Promise<Donor> {
    ...
    const { data, error } = await supabase
      .from('donors')
      .insert({ name: newDonorName.trim(), organization: newDonorOrg.trim() || null, email: newDonorEmail.trim() || null, phone: newDonorPhone.trim() || null })
      .select()
      .single();
  ...
  async function uploadPhoto(): Promise<string | undefined> {
    ...
    const { error } = await supabase.storage.from('item-photos').upload(filename, photoFile, { contentType: photoFile.type });
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const gcDescription = retailer && faceValue ? `${retailer} $${faceValue.toFixed(2)} Gift Card` : `${retailer || 'Gift'} Card`;
  ...
  function resetForm() {
    setStep(1);
    setDonorSearch(''); setDonorResults([]); setSelectedDonor(null); setIsNewDonor(false);
    ...
    return (
      <div className="flex items-center justify-center py-12 px-4">
        <div className="w-full max-w-sm">
          <div className="bg-white rounded-2xl p-6 text-center no-print" style={{ boxShadow: '0 10px 30px -18px rgba(58,31,74,.3)', border: '1px solid rgba(58,31,74,.05)' }}>
            <div className="w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-4" style={{ background: '#e6f4ec' }}>
              <span className="text-3xl">✅</span>
            </div>
            <h2 className="text-xl font-extrabold mb-1" style={{ color: '#3a1f4a' }}>Check-In Complete</h2>
            <p className="text-sm mb-5" style={{ color: '#8a7f92' }}>{successDescription}</p>
            {qrDataUrl && (
              <div className="mb-5">
                <img src={qrDataUrl} alt="QR code" className="mx-auto w-48 h-48" />
                <p className="text-xs mt-2" style={{ color: '#9a8fa0' }}>📱 Scan to go directly to checkout</p>
              </div>
            )}
            <div className="flex gap-3">
              <button
                onClick={() => window.print()}
                className="flex-1 py-2.5 rounded-xl font-semibold transition-colors hover:opacity-80"
                style={{ border: '1.5px solid #d6cee0', color: '#8b4a72', background: '#fff' }}
              >
                🖨️ Print Label
              </button>
              <button
                onClick={resetForm}
                className="flex-1 text-white py-2.5 rounded-xl font-semibold hover:opacity-90 transition-opacity"
                style={{ background: 'linear-gradient(120deg,#f5a623,#d1618a 55%,#7c3a9e)' }}
              >
                New Check-In
              </button>
            </div>
  ...
  return (
    <div className="max-w-lg mx-auto pb-10">
          ...
          <div className="bg-white rounded-2xl p-6" style={{ boxShadow: '0 10px 30px -18px rgba(58,31,74,.3)', border: '1px solid rgba(58,31,74,.05)' }}>
            ...
            {!isNewDonor && !selectedDonor && (
              <>
                <div>
                  ...
                  <input type="text" value={donorSearch} onChange={e => setDonorSearch(e.target.value)} placeholder="Start typing a name…"
                    className="w-full px-4 py-3 rounded-xl text-base focus:outline-none focus:ring-2 focus:ring-purple-300" style={inputStyle} />
                </div>
                {donorResults.length > 0 && (
                  <div className="mt-2 rounded-xl overflow-hidden" style={{ border: '1.5px solid #e3ddec' }}>
                    {donorResults.map(d => (
                      <button key={d.id} onClick={() => { setSelectedDonor(d); setDonorSearch(''); setDonorResults([]); }}
                        className="w-full text-left px-4 py-3 border-b last:border-0 transition-colors" style={{ borderColor: '#f2ede6' }}
                        onMouseEnter={e => (e.currentTarget.style.background = '#f7f4fb')} onMouseLeave={e => (e.currentTarget.style.background = '')}>
                ...
                <button onClick={() => setIsNewDonor(true)} className="mt-4 w-full py-3 rounded-xl text-sm font-bold transition-colors"
            ...
            {isNewDonor && !selectedDonor && (
              <div className="space-y-3">
                ...
                <button onClick={() => setIsNewDonor(false)} className="text-sm font-semibold underline" style={{ color: '#8b4a72' }}>← Back to search</button>
              </div>
            )}
          </div>
        ...
        <div className="space-y-5">
          <div className="bg-white rounded-2xl p-6" style={{ boxShadow: '0 10px 30px -18px rgba(58,31,74,.3)', border: '1px solid rgba(58,31,74,.05)' }}>
            ...
            <div className="space-y-4">
              {/* Item Type Toggle */}
              <div className="flex rounded-xl overflow-hidden" style={{ border: '1.5px solid #e3ddec' }}>
                <button type="button" onClick={() => setItemType('standard')}
                  className="flex-1 py-2.5 text-sm font-semibold transition-colors"
                  style={itemType === 'standard' ? { background: 'linear-gradient(130deg,#1b6cb0,#3f9be0)', color: '#fff' } : { background: '#fbfafd', color: '#4d4457' }}>
                  📦 Standard Donation
                </button>
                <button type="button" onClick={() => setItemType('gift_card')}
                  className="flex-1 py-2.5 text-sm font-semibold transition-colors"
                  style={itemType === 'gift_card' ? { background: 'linear-gradient(130deg,#8b4a72,#7c3a9e)', color: '#fff' } : { background: '#fbfafd', color: '#4d4457' }}>
                  🎁 Gift Card
                </button>
              </div>
              ...
              {itemType === 'gift_card' && (
                <>
                  <div>
                    <label className="block text-sm font-semibold mb-1.5" style={{ color: '#4d4457' }}>Store / Retailer <span className="text-red-500">*</span></label>
                    <input type="text" value={retailer} onChange={e => setRetailer(e.target.value)} placeholder="e.g., Target, Safeway, Chevron…"
                      className="w-full px-4 py-3 rounded-xl text-base focus:outline-none focus:ring-2 focus:ring-purple-300" style={inputStyle} />
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-sm font-semibold mb-1.5" style={{ color: '#4d4457' }}>Face Value per Card ($) <span className="text-red-500">*</span></label>
                      <input type="number" min="0.01" step="0.01" value={faceValue || ''} onChange={e => setFaceValue(parseFloat(e.target.value) || 0)} placeholder="e.g., 25.00"
                        className="w-full px-4 py-3 rounded-xl text-base focus:outline-none focus:ring-2 focus:ring-purple-300" style={inputStyle} />
                    </div>
                    <div>
                      ...
                      <input type="number" min="1" value={quantity} onChange={e => setQuantity(e.target.value === '' ? '' : Math.max(1, parseInt(e.target.value) || 1))}
                        className="w-full px-4 py-3 rounded-xl text-base focus:outline-none focus:ring-2 focus:ring-purple-300" style={inputStyle} />
                    </div>
                  </div>
                  {faceValue > 0 && quantity > 0 && (
                    <div className="rounded-xl p-3 text-sm text-center" style={{ background: '#f3e6f5', border: '1px solid #d6cee0' }}>
                      <span className="font-extrabold" style={{ color: '#3a1f4a' }}>Total Value: ${(faceValue * quantity).toFixed(2)}</span>
                      <span className="ml-2" style={{ color: '#8b4a72' }}>({quantity} × ${faceValue.toFixed(2)})</span>
                    </div>
                  )}
                  <div>
                    <label className="block text-sm font-semibold mb-1.5" style={{ color: '#4d4457' }}>Intended Purpose</label>
                    <select value={gcPurpose} onChange={e => setGcPurpose(e.target.value)}
                      className="w-full px-4 py-3 rounded-xl text-base focus:outline-none focus:ring-2 focus:ring-purple-300" style={selectStyle}>
              ...
              {itemType === 'standard' && (
                <>
                  <div>
                    <label className="block text-sm font-semibold mb-1.5" style={{ color: '#4d4457' }}>Category <span className="text-red-500">*</span></label>
                  ...
                  <div className="relative">
                    <label className="block text-sm font-semibold mb-1.5" style={{ color: '#4d4457' }}>Description <span className="text-red-500">*</span></label>
                    {existingItemId ? (
                      <div className="rounded-xl p-3 flex items-center justify-between" style={{ background: '#e6f4ec', border: '1px solid #a8d5b5' }}>
                        <div>
                          <p className="font-semibold text-sm" style={{ color: '#1a5c36' }}>{description}</p>
                          <p className="text-xs" style={{ color: '#237347' }}>Adding to existing stock — same QR code</p>
                        </div>
                        <button type="button" onClick={() => { setExistingItemId(null); setDescription(''); setCategory(''); setStorageLocation(''); }}
                          className="text-sm ml-3" style={{ color: '#237347' }}>✕</button>
                      </div>
                    ) : (
                      <>
                        <input type="text" value={description} onChange={e => { setDescription(e.target.value); setExistingItemId(null); }}
                          placeholder="e.g., Toilet Paper, Shampoo, Blanket…"
                          className="w-full px-4 py-3 rounded-xl text-base focus:outline-none focus:ring-2 focus:ring-purple-300" style={inputStyle} />
                        {descriptionResults.length > 0 && (
                          <div className="mt-1 rounded-xl overflow-hidden z-10 relative bg-white" style={{ border: '1.5px solid #e3ddec', boxShadow: '0 10px 30px -18px rgba(58,31,74,.3)' }}>
                            <p className="text-xs font-semibold px-3 py-2" style={{ background: '#f3e6f5', color: '#8b4a72', borderBottom: '1px solid #e3ddec' }}>
                              ✓ Existing items — tap to add to current stock
                            </p>
                            {descriptionResults.map(item => (
                              <button key={item.id} type="button"
                                onClick={() => { setDescription(item.description); setCategory(item.category); setStorageLocation(item.storage_location); setExistingItemId(item.id); setDescriptionResults([]); }}
                                className="w-full text-left px-4 py-3 transition-colors" style={{ borderBottom: '1px solid #f2ede6' }}
                                onMouseEnter={e => (e.currentTarget.style.background = '#f7f4fb')} onMouseLeave={e => (e.currentTarget.style.background = '')}>
                                <p className="font-semibold text-sm" style={{ color: '#2a2333' }}>{item.description}</p>
                                <p className="text-xs" style={{ color: '#9a8fa0' }}>{item.category} · {item.storage_location}</p>
                              </button>
                            ))}
                            <button type="button" onClick={() => setDescriptionResults([])}
                              className="w-full text-left px-4 py-2.5 text-xs" style={{ color: '#9a8fa0', borderTop: '1px solid #f2ede6' }}>
                  ...
                  <div>
                    <label className="block text-sm font-semibold mb-1.5" style={{ color: '#4d4457' }}>Storage Location <span className="text-red-500">*</span></label>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-sm font-semibold mb-1.5" style={{ color: '#4d4457' }}>Condition <span className="text-red-500">*</span></label>
                      <select value={condition} onChange={e => setCondition(e.target.value)}
                        className="w-full px-4 py-3 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-purple-300" style={selectStyle}>
                        <option value="">Select…</option>
                        {CONDITIONS.map(c => <option key={c} value={c}>{c}</option>)}
                      </select>
                    </div>
                    <div>
                      <label className="block text-sm font-semibold mb-1.5" style={{ color: '#4d4457' }}>Quantity <span className="text-red-500">*</span></label>
                      <input type="number" min="1" value={quantity} onChange={e => setQuantity(e.target.value === '' ? '' : Math.max(1, parseInt(e.target.value) || 1))}
                        className="w-full px-4 py-3 rounded-xl text-base focus:outline-none focus:ring-2 focus:ring-purple-300" style={inputStyle} />
                    </div>
                  </div>
                  <div>
                    <div className="flex items-center justify-between mb-1.5">
                      <label className="text-sm font-semibold" style={{ color: '#4d4457' }}>
                        FMV Per Unit ($) <span className="text-xs font-normal" style={{ color: '#9a8fa0' }}>auto-estimated — editable</span>
                      </label>
                      <button type="button" onClick={() => setFmvGuideOpen(true)}
                        className="flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-lg transition-colors hover:opacity-80"
                        style={{ background: '#f3e6f5', color: '#8b4a72', border: '1px solid #d6cee0' }}>
                        📋 Value Guide
                      </button>
                    </div>
                    <input type="number" min="0" step="0.01" value={fmvPerUnit} onChange={e => setFmvPerUnit(parseFloat(e.target.value) || 0)}
                      className="w-full px-4 py-3 rounded-xl text-base focus:outline-none focus:ring-2 focus:ring-purple-300" style={inputStyle} />
                    {fmvPerUnit > 0 && quantity > 1 && (
                      <p className="text-xs mt-1 font-semibold" style={{ color: '#237347' }}>Total FMV: ${(fmvPerUnit * quantity).toFixed(2)}</p>
                    )}
                  </div>
                </>
              )}

              {/* Gift card storage location */}
              {itemType === 'gift_card' && (
                <div>
                  <label className="block text-sm font-semibold mb-1.5" style={{ color: '#4d4457' }}>Storage Location <span className="text-red-500">*</span></label>
                  <select value={storageLocation} onChange={e => setStorageLocation(e.target.value)}
              ...
              <div>
                <label className="block text-sm font-semibold mb-1.5" style={{ color: '#4d4457' }}>Date Received</label>
                <input type="date" value={dateReceived} onChange={e => setDateReceived(e.target.value)}
                  className="w-full px-4 py-3 rounded-xl text-base focus:outline-none focus:ring-2 focus:ring-purple-300" style={inputStyle} />
              </div>

              <div>
                <label className="block text-sm font-semibold mb-1.5" style={{ color: '#4d4457' }}>Photo (optional)</label>
                <div onClick={() => fileInputRef.current?.click()}
                  className="rounded-xl p-4 text-center cursor-pointer transition-colors hover:opacity-80"
                  style={{ border: '2px dashed #d6cee0', background: '#fbfafd' }}>
                  {photoPreview ? (
                    <img src={photoPreview} alt="Preview" className="mx-auto max-h-32 rounded-lg object-cover" />
                  ) : (
                    <>
                      <span className="text-2xl">📷</span>
                      <p className="text-sm mt-1" style={{ color: '#8a7f92' }}>Tap to add a photo</p>
                    </>
                  )}
                </div>
                <input ref={fileInputRef} type="file" accept="image/*" capture="environment" onChange={handlePhotoChange} className="hidden" />
                {photoPreview && (
                  <button onClick={() => { setPhotoFile(null); setPhotoPreview(null); }} className="text-xs mt-1 hover:opacity-70" style={{ color: '#c0392b' }}>Remove photo</button>
                )}
              </div>

              <div>
                <label className="block text-sm font-semibold mb-1.5" style={{ color: '#4d4457' }}>Notes (optional)</label>
                <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={2} placeholder="Any additional details…"
                  className="w-full px-4 py-3 rounded-xl text-base focus:outline-none focus:ring-2 focus:ring-purple-300 resize-none" style={inputStyle} />
              </div>
            </div>
          </div>

          {canProceedStep2 && (
            <div className="rounded-2xl p-4 text-sm" style={itemType === 'gift_card'
              ? { background: '#f3e6f5', border: '1px solid #d6cee0' }
              : { background: '#e6f4ec', border: '1px solid #a8d5b5' }}>
              <p className="font-extrabold mb-2" style={{ color: itemType === 'gift_card' ? '#3a1f4a' : '#1a5c36' }}>Summary</p>
              <div className="space-y-1" style={{ color: itemType === 'gift_card' ? '#5d3560' : '#237347' }}>
                <p><span className="font-semibold">Donor:</span> {donorDisplay}</p>
                <p><span className="font-semibold">Program:</span> {program}</p>
                {itemType === 'gift_card' ? (
                  <>
                    <p><span className="font-semibold">Type:</span> 🎁 Gift Card</p>
                    <p><span className="font-semibold">Retailer:</span> {retailer}</p>
                    <p><span className="font-semibold">Cards:</span> {quantity} × ${faceValue?.toFixed(2)} = <strong>${((faceValue ?? 0) * quantity).toFixed(2)}</strong></p>
                    {gcPurpose && <p><span className="font-semibold">Purpose:</span> {gcPurpose}</p>}
                  </>
                ) : (
                  <>
                    <p><span className="font-semibold">Item:</span> {description}</p>
                    <p><span className="font-semibold">Category:</span> {category}</p>
                    <p><span className="font-semibold">Condition:</span> {condition}</p>
                    <p><span className="font-semibold">Qty:</span> {quantity} &nbsp;|&nbsp; <span className="font-semibold">FMV:</span> ${(fmvPerUnit * quantity).toFixed(2)}</p>
                  </>
                )}
                <p><span className="font-semibold">Location:</span> {storageLocation}</p>
              </div>
            </div>
          )}
          ...
          <div className="flex gap-3">
            <button onClick={() => setStep(1)}
              className="flex-1 py-3.5 rounded-xl font-semibold transition-colors hover:opacity-80"
              style={{ border: '1.5px solid #d6cee0', color: '#8b4a72', background: '#fff' }}>
              ← Back
            </button>
            <button onClick={handleSubmit} disabled={!canProceedStep2 || loading}
              className="flex-[2] text-white py-3.5 rounded-xl font-extrabold text-base disabled:opacity-40 disabled:cursor-not-allowed transition-opacity hover:opacity-90"
              style={{ background: 'linear-gradient(120deg,#f5a623,#d1618a 55%,#7c3a9e)', boxShadow: '0 14px 30px -14px rgba(124,58,158,.7)' }}>
      ...
      <FMVGuidePanel open={fmvGuideOpen} onClose={() => setFmvGuideOpen(false)} activeCategory={category} />
    </div>
  );
}
